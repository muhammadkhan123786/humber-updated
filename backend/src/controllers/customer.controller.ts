...
const zeroFill = (periods: string[], data: any[]) => {
  const map = new Map(data.map((item) => [item.period, item]));
...
const domesticServices = new GenericService<CustomerBaseDoc>(
  domesticCutomerSchema,
);

const corporateServices = new GenericService<CustomerBaseDoc>(
  corporateCustomerSchema,
);
...
export const saveCustomer = async (
  ...
  try {
    ...
    const updatedCustomer = await targetService.updateById(id, {
      ...parseResult.data,
      isVatExemption: req.body.isVatExemption,
      vatExemptionReason: req.body.vatExemptionReason,
    });
