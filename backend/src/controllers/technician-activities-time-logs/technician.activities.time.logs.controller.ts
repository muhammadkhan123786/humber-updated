...
import { Parts } from "../../models/master-data-models/parts.models";
...
export const startActivity = async (
  ...
  if (!activity)
    throw new Error("Activity not found or this not your actvity.");
...
export const pauseActivity = async (
  ...
  if (!lastLog.startTime) throw new Error("Invalid session start time");
...
export const resumeActivity = async (
  ...
  const pauseDuration =
    now.getTime() - (lastLog.endTime?.getTime() || now.getTime());
  activity.totalPauseDurationSeconds =
    (activity.totalPauseDurationSeconds || 0) + pauseDuration / 1000;
...
export const getAvailablePartsForActivity = async (
...
export const installPartsDuringActivity = async (
  ...
  try {
    ...
    if (!activity)
      throw new Error("Activity not found or this is not your activity");

    if (!activity.timeLogs.length) throw new Error("No activity log found");
    ...
    const partsFromDB = await Parts.find({
      _id: { $in: partIds },
    });

    const partsMap = new Map(partsFromDB.map((p) => [p._id.toString(), p]));
    ...
    for (const part of partsUsed) {
      ...
      if (!partData) throw new Error(`Part ${part.partId} not found`);

      if (partData.stock < part.quantity) {
        const partName = partData.partName || "Unknown Part";
        throw new Error(`Not enough stock for part (${partName})`);
      }
      ...
      if (!quotationPart)
        throw new Error(`Part ${part.partId} not in quotation`);

      const remainingQty =
        quotationPart.quantity - (quotationPart.installedQuantity || 0);

      if (part.quantity > remainingQty)
        throw new Error(
          `Cannot install ${part.quantity}, only ${remainingQty} left`,
        );

      quotationPart.installedQuantity =
        (quotationPart.installedQuantity || 0) + part.quantity;
    ...
    if (bulkUpdates.length) {
      await Parts.bulkWrite(bulkUpdates);
    }
