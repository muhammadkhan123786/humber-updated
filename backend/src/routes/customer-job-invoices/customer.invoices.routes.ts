...
import { CustomerJobsInvoices, customerInvoiceDoc } from "../../models/invoice-jobs/invoice.jobs.models";
...
const customerInvoiceServices = new GenericService<customerInvoiceDoc>(CustomerJobsInvoices);
...
customerInvoiceRouter.post(
    ...
    (req: Request, _res: Response, next: NextFunction) => {
        if (req.body.invoiceDate) req.body.invoiceDate = new Date(req.body.invoiceDate);
...
customerInvoiceRouter.put("/:id", (req: Request, _res: Response, next: NextFunction) => {
    if (req.body.invoiceDate) req.body.invoiceDate = new Date(req.body.invoiceDate);
    if (req.body.dueDate) req.body.dueDate = new Date(req.body.dueDate);
    next(); // ✅ Important
