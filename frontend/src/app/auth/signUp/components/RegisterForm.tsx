...
import { useForm, FormProvider, useFormContext } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { Eye, EyeOff, Upload, ShieldCheck, Zap, CheckCircle, Lock, MapPin, Building, Globe, Phone, Mail, User, Home, AlertCircle } from 'lucide-react';
import { toast } from 'sonner';
import { registerSchema, RegisterFormValues } from '../schema/registerSchema';
import { useGooglePlacesAutocomplete } from '../hooks/useGooglePlacesAutocomplete';
import useGoogleMapLoad from '@/hooks/useGoogleMapLoad';
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full ${Icon ? 'pl-10' : 'pl-4'} pr-4 py-2.5 bg-gray-50 rounded-lg border ${
            error ? 'border-red-300 focus:ring-red-500' : 'border-gray-200 focus:ring-indigo-500'
          } focus:outline-none focus:ring-2 focus:border-transparent transition ${readOnly ? 'bg-gray-100 cursor-not-allowed' : ''}`}
        />
      </div>
      {error && <p className="text-red-500 text-xs">{error.message as string}</p>}
    </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Loading Maps...</h3>
        <p className="text-sm text-gray-600">Please wait while we initialize Google Maps.</p>
      </div>
    </div>
  );
}
...
export default function RegisterForm() {
  ...
  // State to track if Google Maps API key is valid
  ...
  const { isWorking: isGoogleWorking, isInitialized } = useGooglePlacesAutocomplete(setValue, isGoogleMapsLoaded);

  // Auto switch to manual mode if Google API fails
  useEffect(() => {
    if (isInitialized) {
      if (!isGoogleWorking) {
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="relative lg:w-3/5 p-6 md:p-8 flex flex-col justify-center">
            ...
            <div className="mb-6">
              ...
              {/* Warning message for manual entry */}
              {showManualFallback && (
                <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-2">
                  <AlertCircle className="w-4 h-4 text-yellow-600 mt-0.5 flex-shrink-0" />
                  <div className="text-xs text-yellow-800">
                    <p className="font-medium mb-1">Google Places API is not available</p>
                    <p>Please enter your address details manually in the fields below.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <FormProvider {...methods}>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormInput label="Email Id" name="emailId" type="email" placeholder="you@example.com" required icon={Mail} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormInput label="Shop Name" name="companyName" placeholder="Your Garage" required icon={Building} />
                  <FormInput label="Mobile Number" name="mobileNumber" placeholder="+1 234 567 8900" required icon={Phone} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormInput label="Phone Number" name="phoneNumber" placeholder="+1 234 567 8900" icon={Phone} />
                  <FormInput label="Company Website" name="companyWebsite" placeholder="https://example.com" icon={Globe} />
                </div>

                {/* Address Section - Auto switches between Google and Manual */}
                <div className="space-y-3">
                  <label className="block text-sm font-medium text-gray-700">
                    Address Information <span className="text-red-500">*</span>
                  </label>

                  {/* Google Places Autocomplete (only if API is working) */}
                  {!useManualAddress && isGoogleWorking && (
                    <>
                      <div>
                        <div className="relative">
                          <div className="absolute left-3 inset-y-0 flex items-center pointer-events-none">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormInput label="Latitude" name="latitude" type="number" readOnly />
                        <FormInput label="Longitude" name="longitude" type="number" readOnly />
                      </div>
                    </>
                  )}

                  {/* Manual Address Entry (when Google API fails or not available) */}
                  {(useManualAddress || !isGoogleWorking) && (
                    <>
                      <FormInput 
                        label="Full Address" 
                      ...
                      <p className="text-xs text-gray-500">
                        Note: Latitude and Longitude are optional. You can leave them empty.
                      </p>
                    </>
                  )}
                </div>
