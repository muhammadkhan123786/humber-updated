...
import { getStatusColor, getStatusIcon, getDeliveryStatusBadge } from '../utils/goodsReceivedUtils';
...
export const GRNRow: React.FC<GRNRowProps> = ({
  ...
  const getDeliveryStatusColor = () => {
    if (deliveryStatus === 'Fully Delivered') return 'bg-green-100 text-green-700 border-green-200';
    if (deliveryStatus === 'Partially Delivered') return 'bg-orange-100 text-orange-700 border-orange-200';
    if (deliveryStatus === 'With Rejections') return 'bg-red-100 text-red-700 border-red-200';
  ...
  return (
    ...
    >
      ...
      <td className="p-4">
        <p className="font-medium text-gray-900">{grn.supplier}</p>
      </td>
      <td className="p-4">
        <div className="flex items-center gap-2">
          <Calendar className="h-4 w-4 text-gray-500" />
          <span className="text-gray-700 text-sm">{grn.receivedDate.toLocaleDateString()}</span>
        </div>
      </td>
      <td className="p-4">
        <div className="flex items-center gap-2">
