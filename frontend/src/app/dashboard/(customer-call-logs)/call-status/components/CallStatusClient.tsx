...
export default function CallStatusClient() {
  ...
  const [editingData, setEditingData] = useState<CallStatusWithId | null>(null);
  ...
  const { data, total, isLoading, deleteItem, updateItem } =
    useFormActions<CallStatusWithId>(
      "/call-status",
      "callStatus",
      "Call Status",
      currentPage,
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<CallStatusWithId>("/call-status", {
          limit: "1000",
  ...
  return (
    <div className="min-h-screen ">
      <div className="max-w-6xl mx-auto space-y-6">
        {/* Header - Matching Job Title Gradient */}
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Call Statuses</h1>
              <p className="text-blue-100 text-sm md:text-lg">
                Manage status levels for call records
              </p>
            </div>
          </div>
          <button
        ...
        <StatsCards
          ...
          labels={{
            total: "Total Call Statuses",
            active: "Active Statuses",
            inactive: "Inactive Statuses",
          }}
          icons={{ total: <Activity size={24} /> }}
        />
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                Configure status options for call logs
              </p>
            </div>
