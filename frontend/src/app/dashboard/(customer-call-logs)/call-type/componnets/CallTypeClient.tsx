...
// Assuming callTypeDoc has _id and callTypeName based on your backend
type CallTypeWithId = {
  _id: string;
  callTypeName: string;
  isActive: boolean;
...
export default function CallTypeClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<CallTypeWithId | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<CallTypeWithId>("/call-types", {
          limit: "1000",
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<Phone size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Call Types</h1>
              <p className="text-blue-100 text-sm md:text-lg">
                Manage categories for call logging
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex items-center gap-3 focus-within:ring-2 focus-within:ring-blue-300 transition-all">
          ...
          <input
            type="text"
            placeholder="Search call type name..."
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                Configure call types for support management
              </p>
            </div>
