...
const STATUS_CONFIG: Record<string, { label: string; gradient: string; icon: any }> = {
  pending:     { label: "Pending",    gradient: "from-amber-500 to-orange-500", icon: Clock },
  approved:    { label: "Approved",   gradient: "from-blue-500 to-cyan-500", icon: CheckCircle },
  "in-transit":{ label: "In Transit", gradient: "from-purple-500 to-pink-500", icon: Package },
  completed:   { label: "Completed",  gradient: "from-emerald-500 to-teal-500", icon: CheckCircle },
  rejected:    { label: "Rejected",   gradient: "from-red-500 to-rose-500", icon: XCircle },
};
...
export function ReturnsTab({ supplierId, userId }: Props) {
    ...
    <div className="flex items-center justify-center py-20">
      <div className="relative">
        ...
        <div className="relative bg-white rounded-2xl px-8 py-4 shadow-xl flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-gray-600">Loading returns...</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col items-center justify-center py-20">
      <div className="relative">
        ...
        <div className="relative bg-white rounded-2xl p-12 text-center border-2 border-dashed border-gray-200">
          ...
          <p className="text-sm text-gray-500 mb-6">No goods returns have been created for this supplier.</p>
          <button className="px-6 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl shadow-lg shadow-purple-500/25 hover:shadow-xl transition-shadow">
            Create Return
          </button>
        </div>
      </div>
  ...
  const inProgress = returns.filter((r: any) => ["approved", "in-transit"].includes(r.status)).length;
            ...
            <table className="w-full">
                ...
                <tr className="bg-gradient-to-r from-purple-50/50 to-pink-50/50 border-b border-gray-200">
                  <th className="px-6 py-4 text-left">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold uppercase tracking-wider text-gray-600">Return #</span>
                    </div>
                  </th>
                  <th className="px-6 py-4 text-left">
                    <div className="flex items-center gap-2">
                  ...
                  <th className="px-6 py-4 text-left">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold uppercase tracking-wider text-gray-600">Value</span>
                    </div>
                  </th>
                  <th className="px-6 py-4 text-left">
                    <div className="flex items-center gap-2">
              ...
              <tbody className="divide-y divide-gray-100">
                {returns.map((ret: any) => {
                  ...
                  return (
                    <tr key={ret._id} className="group hover:bg-gradient-to-r hover:from-purple-50/50 hover:to-pink-50/50 transition-all duration-200">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-mono text-sm font-semibold text-gray-800">{ret.grtnNumber}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-gray-800">
                          £{ret.items?.reduce((acc: number, item: any) => acc + (item.value || 0), 0).toFixed(2) || "0.00"}
                        </span>
                      </td>
                      <td className="px-6 py-4">
