        ...
        <div className="p-6 border-b flex justify-between items-start bg-gray-50/50">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className="px-2 py-0.5 text-xs bg-indigo-100 rounded-[10px] text-indigo-700 font-medium border border-indigo-200">
            ...
            <div className="space-y-4">
              {job.services?.length > 0 ? (
                job.services.map((s: any, i: number) => (
                  <div
                    key={i}
                    className="p-5 rounded-2xl border border-gray-100 bg-white shadow-sm hover:border-blue-200 transition-all"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <span className="px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider text-white bg-blue-500">
                          SERVICE
                        </span>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-blue-600 font-black text-lg leading-tight">
                          {s.activityId?.slice(-6) || "N/A"}
                        </p>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-gray-800 font-bold text-lg">
                        {s.description || "Service Task"}
                      </h4>
                      {s.additionalNotes && (
              ...
              {job.parts?.length > 0 ? (
                job.parts.map((p: any, i: number) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <span className="px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider text-white bg-purple-600">
                          S/N: {p.newSerialNumber || "N/A"}
                        </span>
                        <span className="text-gray-400 text-sm font-medium">
                          Qty: {p.quantity || 1}
                        </span>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-emerald-600 font-black text-xl leading-tight">
                          £{p.totalCost?.toLocaleString() || "0.00"}
                        </p>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-gray-800 font-bold text-lg">
                        {p.reasonForChange || "Part Replacement"}
                      </h4>
                      <div className="flex items-center gap-2 text-gray-500 text-sm">
                        ...
                        <span>
                          {p.oldPartConditionDescription || "Standard"}
                        </span>
                      </div>
                    </div>
                  </div>
