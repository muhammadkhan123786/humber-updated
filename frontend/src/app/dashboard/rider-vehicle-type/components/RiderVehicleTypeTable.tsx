...
import { Star, Bike, Trash2 } from "lucide-react";
import { IVehicleType } from "../../../../../../common/master-interfaces/rider.vehicle.type.interface";
import { toast } from "react-hot-toast";

interface Props {
  data: (IVehicleType & { _id: string })[];
  displayView: "table" | "card";
  onEdit: (item: IVehicleType & { _id: string }) => void;
  onDelete: (id: string) => void;
  onStatusChange?: (id: string, newStatus: boolean) => void;
  themeColor: string;
}

// Icon colors for different vehicle types
...
const RiderVehicleTypeTable = ({ data, displayView, onEdit, onDelete, onStatusChange, themeColor }: Props) => {
  // Card View
  if (displayView === "card") {
    return (
        ...
        <tbody className="divide-y divide-gray-100">
          {data.map((item, index) => (
            <tr key={item._id} className="hover:bg-[#ECFEFF] transition-colors">
              ...
              <td className="px-6 py-4 text-center">
                <TableActionButton
                  itemName="vehicle type" // Modal mein likha aayega
                  onEdit={() => onEdit(item)}
                  onDelete={() => {
                    if (item.isDefault) {
...
export default RiderVehicleTypeTable;
