...
import { Bike, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
import StatsCards from "@/app/common-form/StatsCard";
import RiderVehicleTypeTable from "./RiderVehicleTypeTable";
import RiderVehicleTypeForm from "./RiderVehicleTypeForm";
...
type VehicleTypeWithId = IVehicleType & { _id: string };

export default function RiderVehicleTypeClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<VehicleTypeWithId | null>(null);
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<VehicleTypeWithId>(
    "/rider-vehicle-types", 
    "riderVehicleTypes", 
    "Vehicle Type", 
    currentPage, 
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<VehicleTypeWithId>("/rider-vehicle-types", {
          limit: "1000", 
          search: searchTerm.trim(),
        });
        setTotalActiveCount(allDataRes.data?.filter((d: VehicleTypeWithId) => d.isActive).length || 0);
        setTotalInactiveCount(allDataRes.data?.filter((d: VehicleTypeWithId) => !d.isActive).length || 0);
      } catch (err) {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<Bike size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Rider Vehicle Types</h1>
              <p className="text-blue-100 text-sm md:text-lg">Manage vehicle type categories</p>
            </div>
          </div>
          <button
        ...
        <StatsCards 
          ...
          onFilterChange={(filter: 'all' | 'active' | 'inactive') => setFilterStatus(filter)}
          labels={{
            total: "Total Vehicle Types",
            active: "Active Types",
            inactive: "Inactive Types"
          }}
          icons={{ total: <Bike size={24} /> }}
        />
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex items-center gap-3 focus-within:ring-2 focus-within:ring-blue-300 transition-all">
          ...
          <input
            type="text"
            placeholder="Search vehicle type name..."
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              <h2 className="text-2xl font-bold bg-linear-to-r from-blue-600 to-teal-600 bg-clip-text text-transparent">Vehicle Types</h2>
              <p className="text-sm text-gray-500">Configure vehicle type categories for riders</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="text-gray-500 mt-4">Loading vehicle types...</p>
            </div>
          ) : (
            <>
              {filterStatus !== 'all' && (
                <div className="mb-4 text-sm text-gray-600 bg-blue-50 px-4 py-2 rounded-lg">
                  Showing {filterStatus} vehicle types ({filteredDataList.length} results)
                </div>
              )}
              <RiderVehicleTypeTable
