...
import { Package, CheckCircle, XCircle, MinusCircle, Clock, FileText, Loader2 } from "lucide-react";
...
const ListInspections = ({ jobId }: ListInspectionsProps) => {
  ...
  const fetchInspections = async () => {
    ...
    try {
      ...
      const [beforeResult, afterResult] = await Promise.all([beforeResponse, afterResponse]);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12 animate-fadeIn">
        ...
        <p className="text-sm font-medium text-gray-500">Loading inspections...</p>
      </div>
    );
  }
        ...
        >
            ...
            {inspection.inspectionSummary && (
              <div className="bg-white/70 backdrop-blur-sm p-3 rounded-xl border border-gray-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-bold text-gray-600 uppercase">Summary</p>
                </div>
          ...
          <div className="p-5 space-y-3">
            {inspection.inspections.map((item, itemIndex) => (
              <div
                key={item._id}
                style={{ animationDelay: `${(inspectionIndex * 0.1) + (itemIndex * 0.05)}s` }}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h4 className="text-sm font-bold text-gray-900">
                        {item.inspectionTypeId?.technicianInspection || "Inspection Item"}
                      </h4>
                    </div>
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-1">
                ...
                <span className="font-semibold text-gray-700">
                  {inspection.inspections.filter((i) => i.status === "PASS").length} Pass
                </span>
              </div>
              <div className="flex items-center gap-1">
                <XCircle size={14} className="text-red-600" />
                <span className="font-semibold text-gray-700">
                  {inspection.inspections.filter((i) => i.status === "FAIL").length} Fail
                </span>
              </div>
              <div className="flex items-center gap-1">
                <MinusCircle size={14} className="text-gray-600" />
                <span className="font-semibold text-gray-700">
                  {inspection.inspections.filter((i) => i.status === "N/A").length} N/A
                </span>
              </div>
            </div>
