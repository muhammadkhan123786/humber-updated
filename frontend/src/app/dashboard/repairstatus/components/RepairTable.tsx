...
interface Props {
  data: (IRepairStatus & { _id: string })[];
  displayView: "table" | "card";
  onEdit: (item: IRepairStatus & { _id: string }) => void;
...
const RepairTable = ({ data, displayView, onEdit, onDelete, onStatusChange }: Props) => {
  // Card View
  if (displayView === "card") {
    return (
            ...
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="pt-4">
                <TableActionButton
                  ...
                  onDelete={() => {
                    if (item.isDefault) {
                      return toast.error("Default statuses cannot be deleted.");
                    }
                    onDelete(item._id);
                  }}
                />
            ...
            <tr key={item._id} className="hover:bg-[#ECFEFF] transition-colors">
              ...
              <td className="px-6 py-4 text-center">
                <TableActionButton
                  ...
                  onEdit={() => onEdit(item)}
                  onDelete={() => {
                    if (item.isDefault) return toast.error("Default statuses cannot be deleted.");
                    onDelete(item._id);
                  }}
                />
              </td>
