...
interface Props {
  ...
  onEdit: (item: IRepairStatus) => void;
...
const RepairTable = ({ data, displayView, onEdit, onDelete, themeColor }: Props) => {
  if (displayView === "card") {
    return (
        ...
        {data.map((item, index) => (
          ...
          >
            ...
            <div className="px-4 pb-4 space-y-3">
              <h3 className="text-lg font-bold text-gray-900">{item.repairStatus}</h3>
              <div className="flex gap-2 pt-4">
                <button
                  onClick={() => onEdit(item)}
                  className="flex-1 text-sm py-1 px-3 text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg font-semibold transition-all hover:text-orange-600"
                >
                  Edit
                </button>
                <button
                  onClick={() => !item.isDefault && onDelete(item._id)}
                  className={`p-2 rounded-lg transition-all ${item.isDefault ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-red-600 hover:bg-red-50 bg-gray-50'}`}
                >
                  <Trash2 size={20} />
                </button>
              </div>
