...
import TechnicianInspection from "../../jobs/components/TechnicianInspectionModal";
import { jobsAnimations } from "../../jobs/components/JobsAnimation";
import JobInfo from "../../jobs/components/JobInfo";
import Services from "../../jobs/components/Services";
import Parts from "../../jobs/components/Parts";
import axios from "axios";
import TechniciansActivity from "../../jobs/components/TechniciansActivity";

const BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:4000/api";

export type ActivityTabType = 'jobinfo' | 'services' | 'parts' | 'inspection' | 'notes' | 'technician-activities';

interface SharedJobActivityViewProps {
  assignment: any; // The shared job assignment
  onClose: () => void;
}

const SharedJobActivityView = ({ assignment, onClose }: SharedJobActivityViewProps) => {
  ...
  const loadInspectionCounts = async (inspectionTime?: "BEFORE SERVICE" | "AFTER SERVICE") => {
    try {
      ...
      console.log("Inspection types response:", typesResponse.data);
      ...
      if (inspectionTime) {
        try {
          ...
          console.log(`Inspections for ${inspectionTime}:`, response.data);
        ...
        try {
          ...
          console.log("Before service inspections:", beforeResponse.data);
        ...
        try {
          ...
          console.log("After service inspections:", afterResponse.data);
      ...
      case 'notes':
        return (
          <div className="p-6">
            ...
            {/* Shared Job Specific Notes */}
            {assignment.generalNotes && (
              <div className="mb-6 p-4 bg-teal-50 border border-teal-200 rounded-lg">
                <p className="text-sm font-semibold text-teal-800 mb-2">Shared Job Notes (Role: {assignment.role})</p>
                <p className="text-sm text-gray-700">{assignment.generalNotes}</p>
              </div>
            )}
            ...
            {job?.adminNotes && (
              <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm font-semibold text-blue-800 mb-2">Admin Notes</p>
                <p className="text-sm text-gray-700">{job.adminNotes}</p>
              </div>
            )}
            
            <textarea
              placeholder="Add notes for this shared job..."
      ...
      <div className="min-h-screen bg-gray-50 animate-slideUp">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-10">
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Shared Job Activity</h1>
                <p className="text-sm text-gray-500">
                  Job ID: {job?.jobId} • Role: <span className="font-semibold text-teal-600">{assignment.role}</span>
                </p>
              </div>
            </div>
            <button
        ...
        <div className="bg-linear-to-b from-gray-50 to-white border-b border-gray-200 sticky top-[73px] z-10">
          <div className="max-w-7xl mx-auto px-6 py-4">
            <div className="flex items-center justify-center gap-2 bg-linear-to-r from-teal-100 to-cyan-100 rounded-xl shadow-sm p-1">
              {tabs.map((tab) => {
                const Icon = tab.icon;
...
export default SharedJobActivityView;
