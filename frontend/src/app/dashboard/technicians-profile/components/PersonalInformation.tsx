  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await axios.get(JOB_TYPES_API_URL, {
          headers,
          params: { userId },
        });
        if (res.data?.data) {
          const activeJobTypes = res.data.data.filter(
            (j: JobType) => j.isActive
          );
          ...
          if (defaultType && !formData.jobType) {
            setFormData((prev: any) => ({
              ...prev,
              jobType: defaultType.jobTypeName,
            }));
          }
        }
      } catch (err) {
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="w-full lg:w-auto flex flex-col items-center gap-3 p-4 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 min-w-[180px]">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-white hover:bg-gray-100 text-gray-700 text-xs font-bold py-2 px-4 rounded-xl border border-gray-200 transition-all shadow-sm active:scale-95"
          >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                name="role"
                required
                className="w-full p-3 pl-10 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-[#FE6B1D] appearance-none cursor-pointer"
                value={formData.role}
                onChange={handleChange}
              >
                <option value="">
                ...
                {roles.map((r) => (
                  <option key={r._id} value={r.technicianRole}>
                    {r.technicianRole}
                  </option>
                ))}
              </select>
          ...
          <div>
            ...
            <div className="relative">
              <select
                name="jobType"
                required
                className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-[#FE6B1D] appearance-none cursor-pointer"
