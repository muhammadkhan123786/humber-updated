...
const ListAllQuotations = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const quotationsData = Array.isArray(quotationsRes.tickets)
        ? quotationsRes.tickets
        : [];
      const statusesData = Array.isArray(statusesRes.data)
        ? statusesRes.data.filter(
            (s: QuotationStatus) => s.isActive && s.canChooseTechnician,
          )
        : [];

      // Map quotationStatus name to quotationStatusId for each quotation
      const mappedQuotations = (quotationsRes.data || []).map(
        (quotation: QuotationFromBackend) => {
          if (!quotation.quotationStatusId && quotation.quotationStatus) {
            ...
            const matchedStatus = statusesData.find(
              (s: QuotationStatus) =>
                s.ticketQuationStatus?.toLowerCase() ===
                quotation.quotationStatus?.toLowerCase(),
            );
            if (matchedStatus) {
  ...
  const filterQuotations = () => {
    ...
    if (selectedStatus) {
      filtered = filtered.filter((q) => {
        return (
          q.quotationStatus?.toLowerCase() === selectedStatus.toLowerCase()
        );
      });
    }
  ...
  const getStatusInfo = (
    quotation: any,
  ): { name: string; color: string; bgColor: string; statusColor?: string } => {
    ...
    const matchedStatus = quotationStatuses.find(
      (s) =>
        s?.ticketQuationStatus &&
        statusName &&
        s.ticketQuationStatus.toLowerCase() === statusName.toLowerCase(),
    );
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      const selectedStatus = quotationStatuses.find(
        (s) => s._id === newStatusId,
      );
      if (!selectedStatus) {
  ...
  return (
    <div className="shadow-xl bg-white/80 backdrop-blur-sm border-t-4 border-indigo-600 p-6 animate-fadeIn">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* View Modal */}
        {viewingQuotationId && (
          <View quotationId={viewingQuotationId} onClose={handleCloseView} />
        )}
