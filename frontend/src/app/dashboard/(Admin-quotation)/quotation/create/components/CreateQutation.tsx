...
const CreateQuotationPage = () => {
  ...
  const [technicianIdFromQuotation, setTechnicianIdFromQuotation] =
    useState<string>("");
  const [selectedTicket, setSelectedTicket] = useState<any>(null);
  const [showTicketInfo, setShowTicketInfo] = useState(false);
  const [selectedParts, setSelectedParts] = useState<SelectedPart[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [originalQuotationStatus, setOriginalQuotationStatus] =
    useState("SENT TO ADMIN");
  ...
  const loadEditQuotation = async (id: string) => {
    try {
      ...
      if (
        ...
        if (quotation.quotationStatusId) {
          setOriginalQuotationStatus(quotation.quotationStatusId);
          console.log(
        ...
        console.log("quotation edit", quotation);
        if (quotation.ticketId) {
          ...
          const ticketInfo = {
            ...
            ticketStatus:
              ticketData.ticketStatusId?.code ||
              ticketData.ticketStatusId?.ticketStatus ||
              "Unknown",
        ...
        if (quotation.partsList && Array.isArray(quotation.partsList)) {
          console.log("Raw partsList from API:", quotation.partsList);
          ...
          quotation.partsList
            ...
            .forEach((part: any) => {
              const partData =
                typeof part.partId === "object" ? part.partId : part;
              ...
              if (partsMap.has(partId)) {
                ...
                partsMap.set(partId, {
                  _id: partId,
                  partName: partData.partName || "Unknown Part",
                  ...
                  isActive:
                    partData.isActive !== undefined ? partData.isActive : true,
                });
              }
            });

          const formattedParts = Array.from(partsMap.values());
          console.log("Formatted parts for component:", formattedParts);

          setSelectedParts(formattedParts);
        } else {
          console.log("No partsList found or partsList is not an array");
        }
  ...
  const fetchQuotationAutoCode = async () => {
    try {
      const response = await getAll<{ quotationAutoCode: string }>(
        "/quotations/quotation-auto-code",
      );
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-6">
                <QuotationSummary
                  ...
                  originalStatus={originalQuotationStatus}
                />
              </div>
            </div>
          </div>
