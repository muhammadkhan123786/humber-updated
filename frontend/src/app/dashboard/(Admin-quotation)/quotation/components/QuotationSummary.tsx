...
const QuotationSummary = ({
  ...
  const createOrUpdateQuotation = async () => {
    ...
    try {
      const technicianId =
        ...
        selectedTicket.assignedTechnician?._id;
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      doc.text(quotationAutoId || "QUOT-DRAFT", pageWidth / 2, 30, {
        align: "center",
      });
      ...
      const customerName =
        `${selectedTicket.customer?.firstName || ""} ${selectedTicket.customer?.lastName || ""}`.trim();
      if (customerName) {
      ...
      doc.text(`£${total.toFixed(2)}`, pageWidth - 20, yPos + 8, {
        align: "right",
      });
