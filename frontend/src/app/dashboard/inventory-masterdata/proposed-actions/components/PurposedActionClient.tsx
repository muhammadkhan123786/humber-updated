...
const API_URL = `${process.env.NEXT_PUBLIC_API_BASE_URL}/proposed-actions`;
export default function PurposedActionClient() {
  const [dataList, setDataList] = useState<IProposedActions[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<IProposedActions | null>(null);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            >
              <Zap size={36} /> Proposed Actions
            </h1>
            <p className="text-gray-500 mt-1">
              Manage follow-up actions for your leads
            </p>
          </div>
          <button
            onClick={() => {
