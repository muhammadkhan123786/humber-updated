...
interface Props {
  editingData: IProposedActions | null;
  onClose: () => void;
  onRefresh: () => void;
  themeColor: string;
  apiUrl: string;
...
const PurposedActionForm = ({
  ...
  const [formData, setFormData] = useState<
    Pick<IProposedActions, "proposedActionName" | "isActive" | "isDefault">
  >(() => ({
    proposedActionName: editingData?.proposedActionName || "",
      ...
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        >
          <h2 className="text-xl font-bold flex items-center gap-2">
            <Zap size={24} />
            {editingData ? "Edit Proposed Action" : "Add Proposed Action"}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            ...
            <input
              required
              placeholder="e.g. Call Customer, Send Quote"
              className="w-full border rounded-xl p-3 outline-none focus:ring-2 transition-all border-gray-200"
              value={formData.proposedActionName}
              onChange={(e) =>
                setFormData({ ...formData, proposedActionName: e.target.value })
              }
            />
          </div>
          ...
          <button
            ...
            style={{ backgroundColor: themeColor }}
          >
            <Save size={20} /> {editingData ? "Update Action" : "Save Action"}
          </button>
        </form>
      </div>
    </div>
...
export default PurposedActionForm;
