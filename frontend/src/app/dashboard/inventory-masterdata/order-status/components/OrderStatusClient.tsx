...
import { ClipboardList, Plus, Search, Loader2, Grid3x3, List } from "lucide-react";
...
export default function OrderStatusClient() {
  const [dataList, setDataList] = useState<OrderStatusWithId[]>([]);
  const [filteredDataList, setFilteredDataList] = useState<OrderStatusWithId[]>([]);
  ...
  const fetchData = useCallback(async (page = 1, search = "") => {
    try {
      setLoading(true);
      const res = await getAll<OrderStatusWithId>("/order-status", {
        page: page.toString(),
        limit: "12",
        search: search.trim(),
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteItem("/order-status", id);
 ...
 const handleStatusChange = (id: string, newStatus: boolean) => {
  // Generic function call: Instant UI update with background sync
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! shadow-sm rounded-b-2xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            ...
            <div className="flex gap-2 bg-linear-to-r from-gray-100 to-gray-200 rounded-xl p-1 w-full md:w-auto">
              <button onClick={() => setDisplayView("card")} className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${displayView === "card" ? "bg-linear-to-r from-blue-500 to-teal-600 text-white shadow-lg" : "text-gray-600 hover:text-gray-900"}`}>
                <Grid3x3 size={16} /> <span className="text-sm">Grid</span>
              </button>
              <button onClick={() => setDisplayView("table")} className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${displayView === "table" ? "bg-linear-to-r from-blue-500 to-teal-600 text-white shadow-lg" : "text-gray-600 hover:text-gray-900"}`}>
                <List size={16} /> <span className="text-sm">Table</span>
              </button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <OrderStatusTable
                ...
                onEdit={(item) => {
                  setEditingData(item as OrderStatusWithId);
                  setShowForm(true);
                }}
