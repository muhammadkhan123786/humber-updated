...
const API_URL = `${BASE_URL}/order-status`;

export default function OrderStatusClient() {
  const [dataList, setDataList] = useState<IOrderStatus<string>[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<IOrderStatus<string> | null>(
    null
  );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            >
              <ClipboardList size={36} /> Order Statuses
            </h1>
            <p className="text-gray-500 mt-1">
              Configure lifecycle stages for your customer orders
            </p>
          </div>
          <button
            onClick={() => {
        ...
        {(showForm || editingData) && (
          <OrderStatusForm
            key={editingData?._id ?? "new"} // <-- Add key here!
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-20 gap-3">
            ...
            <p className="text-gray-400 font-medium">Loading statuses...</p>
          </div>
        ) : (
          <>
            <OrderStatusTable
