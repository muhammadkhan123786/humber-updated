...
export default function SizeClient() {
  const [dataList, setDataList] = useState<SizeWithId[]>([]);
  const [filteredDataList, setFilteredDataList] = useState<SizeWithId[]>([]);
  ...
  const fetchData = useCallback(async (page = 1, search = "") => {
    try {
      ...
      const res = await getAll<SizeWithId>(API_URL, {
        page: page.toString(),
        limit: "12",
        search: search.trim(),
      ...
      const allDataRes = await getAll<SizeWithId>(API_URL, {
