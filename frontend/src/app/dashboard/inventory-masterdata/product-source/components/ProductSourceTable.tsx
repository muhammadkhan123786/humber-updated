...
interface TableProps {
    data: IProductSource[];
    onEdit: (item: IProductSource) => void;
    onDelete: (id: string) => void;
    themeColor: string;
}

export default function ProductSourceTable({ data, onEdit, onDelete, themeColor }: TableProps) {
    return (
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                    {data.length > 0 ? data.map((item: IProductSource) => (
                        <tr key={item._id} className="hover:bg-orange-50 transition-colors">
                            <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="font-bold text-gray-800">{item.productSource}</span>
                                </div>
                            </td>
                        ...
                        <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-400 italic">No sources found</td></tr>
                    )}
                </tbody>
            </table>
        </div>
