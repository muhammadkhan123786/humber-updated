...
export default function WareHouseClient() {
  const [dataList, setDataList] = useState<IWarehouseStatus[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<IWarehouseStatus | null>(null);
  ...
  const fetchData = async (page = 1, search = "") => {
    try {
      setLoading(true);
      const res = await fetchWarehouseStatuses(page, 10, search);
      if (res?.data) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteWarehouseStatus(id);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            >
              <Box size={36} /> Warehouse Statuses
            </h1>
            <p className="text-gray-500 mt-1">
              Configure status labels for your inventory
            </p>
          </div>
