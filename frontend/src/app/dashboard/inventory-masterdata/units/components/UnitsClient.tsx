...
import { Ruler, Plus, Search, Loader2, Grid3x3, List } from "lucide-react";
...
export default function UnitsClient() {
  const [dataList, setDataList] = useState<UnitWithId[]>([]);
  const [filteredDataList, setFilteredDataList] = useState<UnitWithId[]>([]);
  ...
  const fetchData = useCallback(async (page = 1, search = "") => {
    try {
      ...
      const res = await getAll<UnitWithId>(API_URL, {
        page: page.toString(),
        limit: "12",
        search: search.trim(),
      ...
      const allDataRes = await getAll<UnitWithId>(API_URL, {
...
const handleStatusChange = (id: string, newStatus: boolean) => {
  // Generic function jo UI ko instantly toggle karega aur background mein sync karega
