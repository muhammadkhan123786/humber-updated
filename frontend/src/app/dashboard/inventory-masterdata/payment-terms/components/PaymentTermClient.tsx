...
const API_URL = `${BASE_URL}/payment-terms`;

type PaymentTermWithId = any & { _id: string };

export default function PaymentTermClient() {
  const [dataList, setDataList] = useState<PaymentTermWithId[]>([]);
  const [filteredDataList, setFilteredDataList] = useState<PaymentTermWithId[]>([]);
  ...
  const totalTerms = dataList.length;
  const activeTerms = dataList.filter((d) => d.isActive).length;
  const inactiveTerms = dataList.filter((d) => !d.isActive).length;
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <StatsCards 
          ...
          activeCount={activeTerms}
          inactiveCount={inactiveTerms}
