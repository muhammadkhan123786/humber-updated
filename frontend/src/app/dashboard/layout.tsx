...
<AnimatePresence>
  {sidebarOpen && (
    <div className="fixed inset-0 z-[100] lg:hidden"> {/* High Z-Index is critical */}
      {/* 1. Backdrop Overlay */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-6 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-white tracking-tight">Humber Mobility</span>
          </div>
          <button 
            onClick={() => setSidebarOpen(false)} 
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {navBarLinks.map((item, index) => renderNavItem(item, index, true))}
        </nav>
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="flex flex-col flex-1 bg-gradient-to-br from-indigo-600 via-purple-600 to-purple-700 shadow-2xl overflow-y-auto">
          <div className="flex h-16 items-center px-4 border-b border-white/10 text-white gap-2">
            <Zap className="h-5 w-5" /> <h1 className="text-xl font-semibold">Humber Mobility</h1>
          </div>
