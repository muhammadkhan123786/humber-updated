'use client'
import { Clock, User, Package, Wrench, Calendar, Phone, ArrowRight } from 'lucide-react'
    ...
    <>
      <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg hover:scale-105 transition-all duration-300 ">
        ...
        <div className="p-5 pt-12">
          ...
          {/* Footer Info */}
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-4 text-xs text-gray-600 border-t border-gray-100 space-y-1 pt-3">
            {ticket.ticketSource && (
              <div className="flex items-center gap-1">
                <Phone className="w-3.5 h-3.5 " />
                <span className="capitalize">{ticket.ticketSource}</span>
              </div>
            )}
            ...
            {ticket.assignedTechnician && (
              <div className="flex items-center gap-1 justify-end">
                ...
                <span className="text-indigo-600 font-medium">{ticket.assignedTechnician.firstName} {ticket.assignedTechnician.lastName}</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-end pt-3">
            ...
            >
              View Details
                <ArrowRight className=" mt-0.5 w-4 h-4 text-indigo-600" />
            </button>
          </div>
        </div>
