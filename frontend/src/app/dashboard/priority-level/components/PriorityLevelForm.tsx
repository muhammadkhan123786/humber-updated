...
const PriorityLevelForm = ({ editingData, onClose, onRefresh, themeColor, apiUrl }: Props) => {
    const [formData, setFormData] = useState({
        serviceRequestPrioprity: "", // Backend key matching
        isActive: true,
        isDefault: false,
    });
            ...
            <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="p-6 text-white flex justify-between items-center" style={{ backgroundColor: themeColor }}>
                    <h2 className="text-xl font-bold flex items-center gap-2">
                        <AlertTriangle size={24} /> {editingData ? "Edit Priority" : "Add Priority Level"}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div>
                        <label className="block text-sm font-semibold mb-2 text-gray-700">Priority Name *</label>
                        <input
                            required
                            placeholder="e.g. Urgent, Normal, High"
                            ...
                            onChange={(e) => setFormData({ ...formData, serviceRequestPrioprity: e.target.value })}
                        />
                    </div>
                    ...
                    <button type="submit" className="w-full text-white py-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all" style={{ backgroundColor: themeColor }}>
                        <Save size={20} /> {editingData ? "Update Priority" : "Save Priority"}
                    </button>
                </form>
            </div>
        </div>
