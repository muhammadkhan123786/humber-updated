...
interface Props {
  data: (ITechnicianJobStatus & { _id: string })[];
  displayView: "table" | "card";
  onEdit: (item: ITechnicianJobStatus & { _id: string }) => void;
...
const TechnicianJobTable = ({ data, displayView, onEdit, onDelete, onStatusChange }: Props) => {
  if (displayView === "card") {
    return (
          ...
          <div key={item._id} className="bg-white rounded-3xl border-2 border-blue-200 overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 hover:border-blue-400 hover:scale-105 hover:-translate-y-3 cursor-pointer transform">
            ...
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="flex gap-2 pt-4">
                ...
                <button onClick={() => !item.isDefault ? onDelete(item._id) : alert("Default status cannot be deleted.")} className="p-2 bg-gray-50 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
                  <Trash2 size={20} />
                </button>
              </div>
            </div>
      ...
      <table className="w-full text-left min-w-max">
        <thead className="bg-[#ECFEFF] text-[#364153] border-b-2 border-gray-200 sticky top-0">
          <tr>
            ...
            <th className="px-6 py-4 font-bold">Status Name</th>
          ...
          {data.map((item, index) => (
            <tr key={item._id} className="hover:bg-[#ECFEFF] transition-colors">
              ...
              <td className="px-6 py-4 text-center">
                <TableActionButton 
                    onEdit={() => onEdit(item)} 
                    onDelete={() => item.isDefault ? alert("Default status cannot be deleted.") : onDelete(item._id)} 
                />
              </td>
            </tr>
          ))}
