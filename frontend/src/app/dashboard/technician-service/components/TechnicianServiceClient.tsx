...
const API_URL = "/technician-service-types";
...
export default function TechnicianServiceClient() {
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<ServiceTypeWithId>(
    API_URL, 
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<ServiceTypeWithId>(API_URL, {
          limit: "1000", 
    ...
    <div className="min-h-screen ">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Technician Service Types</h1>
              <p className="text-blue-100 text-sm md:text-lg">Manage technician service categories</p>
            </div>
          </div>
          <button onClick={() => { setEditingData(null); setShowForm(true); }} className="flex items-center justify-center gap-2 text-blue-600 bg-white hover:bg-white/90 px-5 py-2 rounded-lg text-sm h-9 font-semibold shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 w-full md:w-auto">
            <Plus size={22} /> Add Service Type
          </button>
        </div>

        <StatsCards totalCount={total} activeCount={totalActiveCount} inactiveCount={totalInactiveCount} onFilterChange={setFilterStatus} 
          labels={{
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex items-center gap-3 focus-within:ring-2 focus-within:ring-blue-300">
          ...
          <input type="text" placeholder="Search service types..." className="w-full outline-none text-lg" value={searchTerm} onChange={(e) => {
        ...
        <div className="bg-white p-5 pt-9 border-t-4 border-[#2B7FFF]">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">Configure technician service categories and types</p>
            </div>
            <div className="flex gap-2 bg-linear-to-r from-blue-50 to-cyan-50 p-1 rounded-lg border border-blue-200 w-full md:w-auto">
              <button onClick={() => setDisplayView("card")} className={`flex-1 md:flex-none px-3 h-8 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${
                ...
                }`}><LayoutGrid size={16} /> <span className="text-sm">Grid</span></button>
              <button onClick={() => setDisplayView("table")} className={`flex-1 md:flex-none px-3 h-8 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${
                ...
                }`}><Table2 size={16} /> <span className="text-sm">Table</span></button>
            </div>
          </div>

          {showForm && <TechnicianServiceForm editingData={editingData} onClose={() => setShowForm(false)} onRefresh={() => {}} themeColor={THEME_COLOR} />}
          ...
          {isLoading ? (
            ...
            <>
              ...
              {filteredDataList.length > 0 && <div className="mt-6"><Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={(page) => setCurrentPage(page)} /></div>}
            </>
          )}
        </div>
      </div>
