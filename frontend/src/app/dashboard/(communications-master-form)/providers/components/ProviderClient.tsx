...
type ProviderWithId = IProviders & { _id: string };

export default function ProviderClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<ProviderWithId | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<ProviderWithId>("/channel-providers", {
          limit: "1000",
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              ...
              <p className="text-blue-100 text-sm md:text-lg">
                Manage communication channel providers
              </p>
            </div>
          </div>
          {/* <button
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                Manage providers for communication channels
              </p>
            </div>
