...
import { Search, Loader2, LayoutGrid, Table2, Plus, Radio } from "lucide-react";
...
export default function ChannelsClient() {
  ...
  const [editingData, setEditingData] = useState<IChannelData | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await getAll<IChannelData>("/channels", {
          limit: "1000",
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<Radio size={32} className="text-white" />} />
            <div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                Configure your communication integration methods
              </p>
            </div>
