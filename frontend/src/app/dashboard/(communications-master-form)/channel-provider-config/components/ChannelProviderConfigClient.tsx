...
import ChannelProviderConfigTable from "./ChannelProviderConfigTable";
import ChannelProviderConfigForm from "./ChannelProviderConfigForm";
...
export default function ChannelProviderConfigClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<ChannelProviderConfig | null>(
    null,
  );
  ...
  const { data, total, isLoading, deleteItem, updateItem } =
    useFormActions<ChannelProviderConfig>(
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<ChannelProviderConfig>(
          "/channel-providers-fields",
          {
            limit: "1000",
    ...
    <div className="min-h-screen">
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              ...
              <p className="text-blue-100 text-sm md:text-lg">
                Configure fields for channel providers
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex items-center gap-3 focus-within:ring-2 focus-within:ring-blue-300 transition-all">
          ...
          <input
            type="text"
            placeholder="Search by type, name, label..."
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]!">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                Manage field configurations for providers
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <ChannelProviderConfigTable
                data={filteredDataList}
                displayView={displayView}
                onEdit={(item) => {
                  setEditingData(item as any);
                  setShowForm(true);
                }}
