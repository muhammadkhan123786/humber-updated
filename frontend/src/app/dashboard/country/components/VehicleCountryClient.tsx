...
const API_URL = `${BASE_URL}/country`;
  ...
  const fetchData = async (page = 1, search = "") => {
    try {
      ...
      const res = await axios.get(API_URL, {
       ...
       console.log(res.data,"country res data");
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-extrabold flex items-center gap-3" style={{ color: THEME_COLOR }}>
              <Globe size={36} /> Countries
            </h1>
            <p className="text-gray-500 mt-1">Manage countries for addresses and services</p>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex items-center gap-3 border border-gray-100 focus-within:ring-2 focus-within:ring-orange-500 transition-all">
          ...
          <input type="text" placeholder="Search country name..." className="w-full outline-none text-lg" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>

        {(showForm || editingData) && (
          <CountryForm editingData={editingData} onClose={() => { setShowForm(false); setEditingData(null); }} onRefresh={() => fetchData(currentPage, searchTerm)} themeColor={THEME_COLOR} apiUrl={API_URL} />
        )}
        ...
        {loading ? (
          ...
          <>
            <CountryTable data={dataList} onEdit={(item) => { setEditingData(item); setShowForm(false); }} onDelete={handleDelete} themeColor={THEME_COLOR} />
