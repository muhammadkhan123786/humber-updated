...
export default function BrandModelInfo({
  formData,
  setFormData,
}: {
  formData: any;
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE_URL}/vehiclebrand?filter=all`, {
  ...
  useEffect(() => {
    ...
    const brandId =
      typeof formData.vehicleBrandId === "object"
        ? formData.vehicleBrandId?._id
        : formData.vehicleBrandId;

    if (brandId) {
      const token = localStorage.getItem("token");
      axios
        .get(`${BASE_URL}/vechilemodel`, {
          headers: { Authorization: `Bearer ${token}` },
  ...
  return (
    <div className="bg-linear-to-br from-white to-gray-50 p-8 rounded-2xl shadow-lg border border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-6 hover:shadow-2xl transition-all animate-fadeInUp animation-delay-100">
      <div className="animate-fadeInUp">
        <label className="text-sm font-bold text-gray-700 block mb-2 bg-linear-to-br from-blue-600 to-purple-600 bg-clip-text ">
          Make *
        </label>
        <select
