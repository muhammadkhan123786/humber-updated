...
import { Gavel, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
import { useFormActions } from "@/hooks/useFormActions";
import { getAll } from "@/helper/apiHelper"; 
import AnimatedIcon  from "@/app/common-form/AnimatedIcon";
import StatsCards from "@/app/common-form/StatsCard"; 
import TicketDecisionTable from "./TicketDecisionTable";
import TicketDecisionForm from "./TicketDecisionForm";
import { IDecision } from "../../../../../../common/master-interfaces/IDecision.interface";

const THEME_COLOR = "var(--primary-gradient)";
type DecisionWithId = IDecision & { _id: string };

export default function TicketDecisionClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<DecisionWithId | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<DecisionWithId>("/ticket-decision", {
          limit: "1000", 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<Gavel size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
                <h1 className="text-3xl font-bold tracking-tight">Ticket Decisions</h1>
                <p className="text-blue-100 mt-1 font-medium">Manage decision outcomes for tickets</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              ...
              <p className="text-gray-500 text-sm">Overview of all available ticket decisions</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="text-gray-500 mt-4 font-medium animate-pulse">Loading decisions...</p>
            </div>
          ) : (
            <>
              <TicketDecisionTable
              ...
              {total > 12 && (
                <div className="flex justify-center items-center gap-4 mt-8 pt-6 border-t border-gray-100">
                  <button
                    ...
                    className="px-4 py-2 text-sm font-semibold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:border-blue-300 hover:text-blue-600  active:scale-95"
                  >
                    Previous
                  </button>
                  <span className="text-sm font-bold text-gray-700 bg-gray-50 px-4 py-2 rounded-lg border border-gray-100">
