...
const ServiceRequestTable = ({ data, displayView, onEdit, onDelete, themeColor }: Props) => {
  if (displayView === "card") {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {data.map((item, index) => (
          <div key={item._id} className="bg-white rounded-3xl border-2 border-orange-100 overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 hover:border-orange-400 hover:scale-105 hover:-translate-y-3 cursor-pointer transform">
            ...
            <div className="px-4 pb-4 space-y-3">
              <h3 className="text-lg font-bold text-gray-900 mb-2">{item.serviceRequestType}</h3>
              <div className="flex gap-2 pt-4">
                ...
                <button onClick={() => !item.isDefault && onDelete(item._id)} className={`p-2 bg-gray-50 rounded-lg transition-all ${item.isDefault ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-red-600 hover:bg-red-50'}`} disabled={item.isDefault}>
                  <Trash2 size={20} />
                </button>
              </div>
            </div>
