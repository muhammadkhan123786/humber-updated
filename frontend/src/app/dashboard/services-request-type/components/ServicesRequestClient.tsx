...
import { Settings2, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
const API_URL = "/service-request-type";
...
export default function ServiceRequestClient() {
  ...
  const [editingData, setEditingData] = useState<ServiceRequestTypeWithId | null>(null);
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<ServiceRequestTypeWithId>(
    API_URL, 
    "serviceRequestTypes", 
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<ServiceRequestTypeWithId>(API_URL, {
          limit: "1000", 
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Service Request Types</h1>
              <p className="text-blue-100 text-sm md:text-lg">Manage request categories</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              <h2 className="text-2xl font-bold bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 bg-clip-text text-transparent">Service Request Type</h2>
