...
const API_URL = `${BASE_URL}/services-zones`;
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-extrabold flex items-center gap-3" style={{ color: THEME_COLOR }}>
              <Map size={36} /> Service Zones
            </h1>
            <p className="text-gray-500 mt-1">Manage geographical areas where your services are available</p>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex items-center gap-3 border border-gray-100 focus-within:ring-2 focus-within:ring-orange-500 transition-all">
          ...
          <input type="text" placeholder="Search zones..." className="w-full outline-none text-lg" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        ...
        {loading ? (
          ...
          <>
            <ServiceZoneTable data={dataList} onEdit={(item) => { setEditingData(item); setShowForm(false); }} onDelete={handleDelete} themeColor={THEME_COLOR} />
