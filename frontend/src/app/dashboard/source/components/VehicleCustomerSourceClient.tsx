...
const API_URL = `${BASE_URL}/customer-source`; // API path as per your backend app.use
...
export default function VehicleCustomerSourceClient() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this customer source?")) return;
    try {
      const token = localStorage.getItem("token");
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-extrabold flex items-center gap-3" style={{ color: THEME_COLOR }}>
              <Share2 size={36} /> Customer Sources
            </h1>
            <p className="text-gray-500 mt-1">Track where your customers are coming from</p>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex items-center gap-3 border border-gray-100 focus-within:ring-2 focus-within:ring-orange-500 transition-all">
          ...
          <input type="text" placeholder="Search sources..." className="w-full outline-none text-lg" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>

        {(showForm || editingData) && (
          <CustomerSourceForm editingData={editingData} onClose={() => { setShowForm(false); setEditingData(null); }} onRefresh={() => fetchData(currentPage, searchTerm)} themeColor={THEME_COLOR} apiUrl={API_URL} />
        )}
        ...
        {loading ? (
          ...
          <>
            <CustomerSourceTable data={dataList} onEdit={(item) => { setEditingData(item); setShowForm(false); }} onDelete={handleDelete} themeColor={THEME_COLOR} />
