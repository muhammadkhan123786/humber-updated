...
const TicketActionTable = ({ data, displayView, onEdit, onDelete, onStatusChange, themeColor }: Props) => {
            ...
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="flex gap-2 pt-4">
                ...
                <button
                  onClick={() => {
                    if (item.isDefault) {
                      alert("Default actions cannot be deleted.");
                      return;
                    }
                    onDelete(item._id);
                  }}
      ...
      <table className="w-full text-[16px]! text-left">
        <thead className="bg-[#ECFEFF] border-b-2 border-gray-200">
          <tr>
            ...
            <th className="px-6 py-4 font-bold text-gray-700">Action Label</th>
            ...
            <tr key={item._id} className="hover:bg-[#ECFEFF] transition-colors">
              ...
              <td className="px-6 py-4 text-center">
                <TableActionButton
                  onEdit={() => onEdit(item)}
                  onDelete={() => {
                    if (item.isDefault) return alert("Default actions cannot be deleted.");
                    onDelete(item._id);
                  }}
                />
              </td>
