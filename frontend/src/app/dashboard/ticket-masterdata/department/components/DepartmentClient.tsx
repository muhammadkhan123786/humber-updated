...
export default function DepartmentClient() {
  // FIXED: Yahan type define ki hai <IDepartments[]>
  const [dataList, setDataList] = useState<IDepartments[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  
  // FIXED: editingData ko interface ya null type diya hai
  const [editingData, setEditingData] = useState<IDepartments | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      // getAlls ko generic type <IDepartments> pass ki hai
      const res = await getAlls<IDepartments>(API_URL, { 
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-extrabold flex items-center gap-3 text-[#FE6B1D]">
            <Building2 size={36} /> Departments
          </h1>
          <p className="text-gray-500 italic">Organize your team into functional units</p>
        </div>
        <button 
      ...
      {loading ? (
        ...
        <DepartmentTable 
          data={dataList} 
          onEdit={(item: IDepartments) => setEditingData(item)} 
