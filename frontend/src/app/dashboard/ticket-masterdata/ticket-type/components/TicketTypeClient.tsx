...
export default function TicketTypeClient() {
  ...
  const [filteredDataList, setFilteredDataList] = useState<IPopulatedTicketType[]>([]);
  ...
  const fetchData = useCallback(async (page = 1, search = "") => {
    try {
      ...
      const res = await getAll<IPopulatedTicketType>(API_URL, {
        page: page.toString(),
        limit: "12",
        search: search.trim(),
  ...
  // Filter logic for Active/Inactive stats
  useEffect(() => {
    if (filterStatus === 'all') {
 ...
 const handleStatusChange = (id: string, newStatus: boolean) => {
  // Generic function call jo status update ko smooth banaye ga
  handleOptimisticStatusUpdate(
    id,
    newStatus,
    API_URL, // Yahan directly API_URL variable pass kar dein
    setDataList,
    setTotalActiveCount,
              ...
              {filterStatus !== 'all' && (
                <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-lg flex items-center justify-between">
                  <span className="text-sm text-orange-700 font-medium">
                    Showing {filterStatus === 'active' ? 'Active' : 'Inactive'} Categories ({filteredDataList.length})
                  </span>
                  <button
