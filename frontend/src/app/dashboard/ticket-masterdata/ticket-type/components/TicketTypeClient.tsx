...
import { Layers, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
export default function TicketTypeClient() {
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<IPopulatedTicketType>(
    API_URL, 
    "ticketTypes", 
    "Ticket Type", 
    currentPage, 
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Ticket Types</h1>
              <p className="text-blue-100 text-sm md:text-lg">Categorize tickets by technical domains</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! rounded-b-2xl shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              <h2 className="text-2xl font-bold bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 bg-clip-text text-transparent">Ticket Categories</h2>
              <p className="text-sm text-gray-500">Configure ticket types for department assignment</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="mt-4 text-gray-400 font-medium">Loading ticket types...</p>
            </div>
          ) : (
            <>
              {filterStatus !== 'all' && (
