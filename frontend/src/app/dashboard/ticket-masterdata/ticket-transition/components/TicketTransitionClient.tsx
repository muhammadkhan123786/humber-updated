...
import { GitCompare, Plus, Loader2, LayoutGrid, Table2, Search } from "lucide-react";
...
const API_URL = "/ticket-transition-setup";
...
export default function TicketTransitionClient() {
  ...
  const [editingData, setEditingData] = useState<PopulatedTransition | null>(null);
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<PopulatedTransition>(
    API_URL, 
    "ticketTransitions", 
    "Ticket Transition", 
    currentPage, 
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<PopulatedTransition>(API_URL, {
          limit: "1000", 
    ...
    <div className="min-h-screen ">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Status Transitions</h1>
              <p className="text-blue-100 text-sm md:text-lg">Define how tickets move between statuses based on actions</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <StatsCards 
          ...
          icons={{ total: <GitCompare size={24} /> }}
        />
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex items-center gap-3 focus-within:ring-2 focus-within:ring-blue-300 transition-all">
          ...
          <input
            type="text"
            placeholder="Search transition rules..."
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              <h2 className="text-2xl font-bold bg-linear-to-r from-blue-600 to-teal-600 bg-clip-text text-transparent">Transition Rules</h2>
              <p className="text-sm text-gray-500">Configure status transitions and workflow rules for tickets</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="mt-4 text-gray-400 font-medium">Loading transitions...</p>
            </div>
          ) : (
            <>
              {filterStatus !== 'all' && (
