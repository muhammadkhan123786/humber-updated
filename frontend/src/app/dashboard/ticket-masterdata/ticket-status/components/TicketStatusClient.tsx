...
export default function TicketStatusClient() {
  ...
  const [editingData, setEditingData] = useState<TicketStatusWithId | null>(
    null
  );
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            >
              <Tag size={36} /> Ticket Statuses
            </h1>
            <p className="text-gray-500">
              Configure workflow stages for the support system
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex items-center gap-3 border border-gray-100">
          ...
          <input
            type="text"
            placeholder="Search status code or label..."
          ...
          <>
            ...
            {dataList.length === 0 && !loading && (
              <div className="text-center py-20 bg-white rounded-2xl border border-dashed">
                <p className="text-gray-400">No ticket statuses found.</p>
              </div>
            )}
          </>
        )}
