...
interface Props {
  data: (ILABOURCOST & { _id: string })[];
  displayView: "table" | "card";
  onEdit: (item: ILABOURCOST & { _id: string }) => void;
  onDelete: (id: string) => void;
  onStatusChange?: (id: string, newStatus: boolean) => void;
  themeColor: string;
}

// Icon colors for different labour rates
...
const LabourTable = ({ data, displayView, onEdit, onDelete, onStatusChange, themeColor }: Props) => {
  // Card View
  if (displayView === "card") {
    return (
            ...
            <div className="px-4 pb-4 space-y-3">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-2xl font-bold text-blue-600">${item.value.toFixed(2)}</span>
              ...
              <div className="pt-4">
                <TableActionButton
                  ...
                  onDelete={() => {
                    if (item.isDefault) {
                      return toast.error("Default rates cannot be deleted.");
                    }
                    onDelete(item._id);
                  }}
                />
      ...
      <table className="w-full text-[16px]! text-left min-w-max">
        <thead className="bg-[#ECFEFF] text=[#364153]! border-b-2 border-gray-200 sticky top-0">
          <tr>
            ...
            <th className="px-6 py-4 font-bold text-gray-700 whitespace-nowrap">Rate Value</th>
