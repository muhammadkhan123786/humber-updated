...
import TechniciansRolestable from "./TechnicionRolesTable";
import TechnicianRolesForm from "./TechnicionRolesForm";
import Pagination from "./Pagination"; 

const THEME_COLOR = "#FE6B1D";
const BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL;
const API_URL = `${BASE_URL}/technician-roles`;

export default function TechniciansRolesClient() {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-extrabold flex items-center gap-3" style={{ color: THEME_COLOR }}>
              <ShieldCheck size={36} /> Technician Roles
            </h1>
            <p className="text-gray-500 mt-1">Define and manage job roles for your technical staff</p>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex items-center gap-3 border border-gray-100 focus-within:ring-2 focus-within:ring-orange-500 transition-all">
          ...
          <input type="text" placeholder="Search roles..." className="w-full outline-none text-lg" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>

        {(showForm || editingData) && (
          <TechnicianRolesForm editingData={editingData} onClose={() => { setShowForm(false); setEditingData(null); }} onRefresh={() => fetchData(currentPage, searchTerm)} themeColor={THEME_COLOR} apiUrl={API_URL} />
        )}
        ...
        {loading ? (
          ...
          <>
            <TechniciansRolestable data={dataList} onEdit={(item) => { setEditingData(item); setShowForm(false); }} onDelete={handleDelete} themeColor={THEME_COLOR} />
