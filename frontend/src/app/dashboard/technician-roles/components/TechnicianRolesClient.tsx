...
import { ShieldCheck, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
export default function TechnicianRolesClient() {
  ...
  const [editingData, setEditingData] = useState<TechnicianRoleWithId | null>(null);
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<TechnicianRoleWithId>(
    "/technician-roles", 
    "technicianRoles", 
    "Technician Role", 
    currentPage, 
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<TechnicianRoleWithId>("/technician-roles", {
          limit: "1000", 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
           <AnimatedIcon icon={<ShieldCheck size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Technician Roles</h1>
              <p className="text-blue-100 text-sm md:text-lg">Define and manage staff roles</p>
            </div>
          </div>
          <button
            onClick={() => {
