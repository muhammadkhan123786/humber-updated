import { Edit, Trash2, Star, UserCheck } from "lucide-react";
...
const TechniciansRolestable = ({ data, onEdit, onDelete, themeColor }: Props) => {
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
      <table className="w-full text-left">
        <thead className="text-white" style={{ backgroundColor: themeColor }}>
          <tr>
            <th className="p-4">Technician Role</th>
        ...
        <tbody className="divide-y divide-gray-100">
          {data.length > 0 ? data.map((item) => (
            <tr key={item._id} className="hover:bg-orange-50 transition-colors">
              <td className="p-4">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-gray-800">{item.technicianRole}</span>
                </div>
              </td>
            ...
            <tr>
              <td colSpan={4} className="p-10 text-center text-gray-400 italic">No technician roles found</td>
            </tr>
          )}
        </tbody>
      </table>
...
export default TechniciansRolestable;