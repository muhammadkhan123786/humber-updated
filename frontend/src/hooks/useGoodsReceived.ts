...
export const useGoodsReceived = () => {
   ...
   * Filters to only "ordered" or "received" status POs
   * ======================================================= */
  ...
  /* =========================================================
   * HANDLE PO SELECTION
   * Reads productName & sku from the populated productId object
   * which is what the backend returns (see document 6 shape)
   * ======================================================= */
  ...
  const handleExportGRN = async (grn: GoodsReceivedNote) => {
    ...
    try {
      ...
      const downloadToast = toast.loading(
        `Generating PDF for ${grn.grnNumber}...`
      );
